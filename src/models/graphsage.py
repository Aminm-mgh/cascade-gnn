"""
GraphSAGE model for cascade-gnn.

Same architecture pattern as GCN, but uses SAGEConv, which separately
transforms a node's own features and its aggregated neighbor features,
...
class SAGEEncoder(torch.nn.Module):
    def __init__(self, hidden_dim=32):
        ...
        self.conv1 = HeteroConv({
            ("customer", "orders_from", "category"): SAGEConv(-1, hidden_dim, aggr="mean",normalize=True),
            ("category", "rev_orders_from", "customer"): SAGEConv(-1, hidden_dim, aggr="mean",normalize=True),
        }, aggr="mean")

        self.conv2 = HeteroConv({
            ("customer", "orders_from", "category"): SAGEConv(hidden_dim, hidden_dim, aggr="mean",normalize=True),
            ("category", "rev_orders_from", "customer"): SAGEConv(hidden_dim, hidden_dim, aggr="mean",normalize=True),
        }, aggr="mean")
...
class SAGEModel(torch.nn.Module):
    def __init__(self, hidden_dim=32, edge_feat_dim=6):
        super().__init__()
        self.encoder = SAGEEncoder(hidden_dim)
...
if __name__ == "__main__":
    ...
    print("--- Final Test Set Performance (GraphSAGE) ---")
